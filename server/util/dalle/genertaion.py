...
from util.firestore.firestore import add_generated_image_document
from util.gcp.secret import access_secret_version
from util.gcp.storage.file import upload_blob_from_memory, generate_download_signed_url_v4
...
def generate_image(prompt):
    ...
    for image in generation_response["data"]:
        ...
        upload_blob_from_memory('smart-salon-generated-photo', generated_image, generated_image_name)

        add_generated_image_document(generated_image_name, prompt)

        result.append({
            'generated_image_name': generated_image_name,
            'prompt': prompt,
            'url': generate_download_signed_url_v4('smart-salon-generated-photo', generated_image_name)
        })
